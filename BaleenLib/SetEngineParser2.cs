...
".Replace("<<BLN_DB>>", NewDBName);
        ...
        public static void AtomizeDirectory(string sourceDirectoryPath, bool createDB)
        {
            ...
            foreach (FileInfo f in fins)
            {
                AtomizeFile(f.FullName, ref objectID, writers);
            }
        ...
        public static void AtomizeFile(string sourceFile, ref long objectID, Dictionary<string,StreamWriter> writers)
        {
            ...
            while (!sr.EndOfStream)
            {
                ...
                if (dat != null && hdr != null && hdr.Length == dat.Length)
                {
                    for (int i = 0; i < hdr.Length; i++)
                    {
                        ...
                        else if (type is string && d.Length <= 400)
                        {
                            wKey = wKey + "-TXTS";
                        }
                        else if (type is string && d.Length > 400)
